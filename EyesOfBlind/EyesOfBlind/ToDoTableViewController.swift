...
//  EyesOfBlind
//
//  Created by Beth—荧 on 2017/10/6.
...
class ToDoTableViewController: UITableViewController, NSFetchedResultsControllerDelegate,ToDoItemDelegate{
    ...
    override func viewDidLoad() {
        ...
        let client = MSClient(applicationURLString: "https://eyesofblind.azurewebsites.net")
        let managedObjectContext = (UIApplication.shared.delegate as! AppDelegate).managedObjectContext!
    ...
    func configureCell(_ cell: UITableViewCell, indexPath: IndexPath) -> UITableViewCell {
        ...
        if let text = item.value(forKey: "text") as? String {
            let addresss = item.value(forKey: "address") as? String
    ...
    func didSaveItem(_ text: String, addressText: String)
    {
        ...
        let itemToInsert = ["text": text ,"address": addressText,"complete": false, "__createdAt": Date()] as [String : Any]
    ...
    /**
     decide to search item in sql
     */
    func searchItem(_ text: String) -> String {
        self.onRefresh(self.refreshControl)
        var addressText = ""
        print("......................")
        self.table!.read { (result, error) in
            if let err = error {
                print("ERROR ", err)
            } else if let items = result?.items {
                for item in items {
                    let textss = item["text"] as? String
                    if textss == text{
                        addressText = item["address"] as! String
                        self.addressTrans = addressText
